...
describe('User', () => {
  ...
  it('should sign in and sign out', () => {
    ...
    cy.findByText(/gabriel/i).should('exist').click()
    cy.findByText(/sign out/i).click()

    cy.findByRole('link', { name: /sign in/i }).should('exist')
    cy.findByText(/gabriel/i).should('not.exist')
  });
  ...
  it('should sign the user and redirect to the page that it was previously defined', () => {
    ...
    cy.findByLabelText(/username/i).should('have.value', 'Gabriel')
    cy.findByLabelText(/e-mail/i).should('have.value', 'gabrielcastilho70@gmail.com')
  });
});
